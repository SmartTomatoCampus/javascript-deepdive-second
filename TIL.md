...
1. 서버에 리소스(html, css, js, 이미지, 폰트 등) 요청, 서버로 부터 응답 받음
2. html, css 파싱 -> DOM, CSSOM => 렌더 트리
3. JS 파싱 -> AST(추상적 구문트리) -> 바이트코드 변환, 실행
4. 렌더트리 기반 -> 레이아웃계산(위치, 크기), 페인팅

> 서버에 요청하는 방식

    1. URL을 통해서
      - url -> IP 주소 by DNS -> 해당 IP 주소의 서버에 요청 전송
      - cf) 루트요청: 스킴(/)과 호스트만으로 구성된 URI에 의한 요청
      - 정적파일인 index.html을 응답받음

    2. JS를 통해 서버요청 - REST API, ajax ...
      - 정적파일과 동적파일 모두 응답 받을 수 있음

## HTML 파싱 DOM 생성 / CSS 파싱 CSSOM 생성

    - HTML 구조를 브라우저가 이해할 수 있는 형태로 변환하여 메모리에 저장
    - 파싱은 한 줄씩 순차적으로 진행됨
    - DOM 생성 과정
      1. 서버는 요청받은 HTML파일을 바이트로 전달
      2. 브라우저는 meta태그 charset에 의해 인코딩 -> 문자열로 변환
      3. 문자열을 토큰(최소한의 문법적의미를 갖는)으로 분해
      4. 토큰들을 객체로 변환하고 노드들을 생성
      5. 중첩 관계를 갖는 노드들을 트리 자료구조로 구성 => 이것이 DOM!

    - CSSOM 생성 과정
      - HTML과 동일한 과정을 거침(바이트 -> 문자열 -> 토큰 -> 노드 -> 트리구조)
...
## 자바스크립트 파싱과 실행
    ...
    - JS 해석 by JS 엔진 -> AST(추상적구문 트리) 생성 -> 바이트코드 생성, 실행
...
- 이벤트 핸들러 등록 -> 브라우저에게 이벤트 핸들러 호출을 위임

> 이벤트 핸들러 등록 방식

1. 어트리뷰트 방식

- on 접두사로 시작하는 이벤트 타입의 함수를 참조!
- 함수 선언은 평가 결과값이 이벤트 핸들러로 등록되므로 주의
- 이벤트 객체 전달 받을시 첫번째 매개변수는 반드시 event여아 한다. e(X)

2. 프로퍼티 방식

- DOM 노드 객체가 가지고 있는 이벤트 핸들러 프로퍼티로 이벤트 핸들러 등록
...
- 메서드를 통해 이벤트 핸들러 등록
- 이벤트 타깃.(이벤트타입, 이벤트핸들러, capture 사용여부)

> 이벤트 객체

- 이벤트 객체는 이벤트 핸들러의 첫번째 인수로 암묵적으로 전달됨
- 이벤트 객체는 생성자 함수로 임의로 만들수도 있음!

- 이벤트 객체는 공통의 프로퍼티
  - type, target, isTrusted, timeStamp 등

> 마우스 정보 취득

- clientX/clientY는 뷰포트 웹페이지의 가시 영역을 기준으로 마우스 포인터 좌표를 나타냄

> 이벤트 전파

- DOM 요소 노드에서 발생한 이벤트는 DOM 트리를 통해 전파된다.

  - 캡처링 단계 -> 이벤트가 상위 -> 하위 전달
  - 타깃 단계 -> 이벤트가 이벤트 타깃에 도달
  - 버블링 단계 -> 이벤트가 하위 -> 상위 전달

- 이벤트 어트리뷰트/프로퍼티 -> 타깃, 버블링 이벤트만 캐치 가능
- addEventListener -> 타깃, 버블링, 갭처링 이벤트 모두 개치 가능 - 세번째 인수를 true로!

=> 이벤트는 이벤트를 발생시킨 이벤트 타깃은 물론 상위 DOM 요소에서도 캐치가 가능하다!

- 이벤트 전파가 일어나지 않는 이벤트 객체

  - 포커스 -> focus/blur
  - 리소스 -> load/unload/abort/error
  - 마우스 -> mouseenter/mouseleave

- currnetTarget -> 이벤트가 부착된 부모 요소 위치 반환
- event.target -> 클릭한 자식 요소 반환

- preventDefault 메서드 -> DOM 요소의 기본동작 중단시킴
- stopPropagation 메서드 -> 하위 DOM 요소의 이벤트 처리를 개별적으로 하기 위해 이벤트 전파 중단시킴

# 41장: 타이머

- 호출 스케줄링

  - 타이머 함수(setTimeout, setInterval)는 생성한 타이머가 종료되면 콜백함수가 실행됨

- JS는 1개의 실행컨텍스트 -> 싱글 쓰레드 -> 타이머 함수 - 비동기 처리방식

> setTimeout

- 인수 - 콜백함수, delay(1/1000), 콜백함수 인수
- 타이머 id를 반환 -> clearTimeout로 타이머 취소 가능!
- setInterval 함수는 전달받은 시간마다 콜백함수를 반복적으로 실행!
- 인수는 setTImeou과 같음!

> 디바운스와 스로틀

- 짧은 시간 연속해서 발생하는 이벤트를 그룹화해서 과도한 이벤트 핸들러 호출을 방지하는 프로그래밍 기법!

> 타이머 함수의 활용

- 디바운스

  - 특정 시간보다 짧은 이벤트는 처리히지 않다가 나중에 한번에 처리
  - debounce 함수 -> delay 보다 짧으면 타이머 시간을 재설정하여 콜백함수가 실행되지 않게함!
  - 활용 -> resize 이벤트처리, input 입력값 ajax 요청 자동완성 UI 구현, 버튼 중복 클릭 방지 처리 등

- 스로틀
  - 짧은 시간 간격 연속 이벤트를 일정 시간 단위로 이벤트 핸들러가 호출되도록 호출 주기를 만듦
...
- 함수 실행 순서대로 순차적으로 실행
- 함수가 호출된 순서대로 함수 실행 컨텍스트가 실행 컨텍스트 스택에 푸시되기 때문에!
- JS -> 싱글 쓰레드 -> 블로킹 현상(작업중단)발생 할 수 있음

> 비동기 처리

- 실행 중인 태스크 종료 되기 전에라도 다음 태스크 곧바로 실행
- 블로킹 발생X
- 태스크 실행 순서를 보장하지 않음
...
- JS 엔진
  - 콜 스택 -> 실행컨텍스트 스택
  - 힙 -> 객체 저장된 메모리 공간, 객체를 저장(크기 모름)하기 때문에 -> 구조화X
- 소스코드 평가, 실행 -> JS엔진이 처리
- 그 이외의 것들과 콜백 함수의 평가 실행 -> 브라우저 or Node가 담당

  - 이벤트 루프

    -> 콜 스택이 비어있는지 혹은 태스크 큐에 대기중인 함수 확인 후 비어있다면 실행 컨텍스트를 콜 스택으로 푸시시킴
...
- 에러 처리를 해야하는 이유
  ...
  - 발생한 에러에 의해 프로그램이 강제 종료된다.
    -> 예외적인 상황이 발생할 수 있다는 것을 전제하고 코드 작성해야함!

- 에러처리 방법

1. 예외적인 상황에서 반환값으로 if문 처리
2. 에러처리 코드 미리 등록

- try ... catch ... finally ...
- try문 실행, 에러 발생 -> catch(err) 에러 err을 받음 -> finally 에러 상관없이 무조건 한 번 실행

- Error 객체

  - `const error = new Error('invalid')`
  - message 프로퍼티 -> 생성자 함수에 인수로 전달한 메세지
  - stack 프로퍼치 -> 에러를 발생시킨 콜스택 호출 정보 문자열
  - 7개의 에러 객체가 존재함

- throw 문

  - 에러 객체를 던지는 용도 -> catch에서 그 에러를 받음

- 에러 전파
  - 에러는 호출자 방향으로 전파됨(실행 컨텍스트 위에서 밑으로)
    => 전역에서 캐치 가능하다!
